 ...
 * 动车二等卧购票选铺组件
 * <p>
 * 数据模型：每节车厢 9 个隔间 x 4 个铺位（A=左下铺, C=左上铺, D=右下铺, F=右上铺）
 * <p>
 * 与一等卧的区别：
 *   - 一等卧每节车厢 8 个隔间，二等卧每节车厢 9 个隔间（铺位更密集）
 *   - 选铺优先级逻辑完全一致：下铺 > 上铺
 * <p>
 * 选铺策略：
...
public class TrainSecondSleeperPurchaseTicketHandler extends AbstractTrainPurchaseTicketTemplate {
     ...
     * 二等卧车厢布局：9 个隔间
     */
    private static final int COMPARTMENT_COUNT = 9;
     ...
     */
    private static final int SEAT_TYPE_CODE = 5;

    @Override
    public String mark() {
        return VehicleTypeEnum.BULLET.getName() + VehicleSeatTypeEnum.SECOND_SLEEPER.getName();
    }
